...
function handleNullAvatar(img, with_path = "", width = "75") {
    if (img && with_path) {
        ...
        return `<img class='img-fluid rounded-circle' src='/img/noimg.png' width='${width}' alt='image'>`;
    }
}
...
function handleOrderStatus(status) {
    if (status === 0) {
        ...
        return `<span class='badge badge-success'> On Delivery <i class='fas fa-shipping-fast ml-1'></i></span>`;
    } else if (status === 4) {
        return `<span class='badge badge-success'>Delivered <i class='fas fa-times-circle ml-1'></i></span>`;
    } else {
...
/**
 * check if the status is customized or not
 */
function isCustomized(bool) {
    return bool == true
        ? `<span class='badge badge-success'>Yes <i class='fas fa-check-circle ml-1'></i></span>`
        : `<span class='badge badge-danger'>No</span>`;
}
 ...
 *
 */
function handleProductQty(qty) {
    if (qty > 0 && qty <= 10) {
        return `<span class='text-danger'>${qty}</span>`;
    } else if (qty > 10 && qty <= 20) {
        return `<span class='text-warning'>${qty}</span>`;
    } else {
