...
@section('content')
    <div class="container-fluid mt-5">
        ...
        <br>
        <h3 class="text-center">Top Selling Product</h3> <br>
        <form class="d-print-none" action="{{ route('admin.print.handle') }}" method="GET">
            <div class="input-group">
                <input type="hidden" name="records" value="top_selling_product">
                <input type="hidden" name="execute" value="1">
                <select class="form-control" name="rows">
                    <option value="{{ \App\Models\Order::where('status', 2)->count() }}">--- All Rows--- </option>
        ...
        <table class="table table-flush table-hover">
            ...
            <tbody>
                @forelse (array_combine($top_selling_product[0], $top_selling_product[1]) as $product => $total_order)
                    <tr>
                        <td>{{ $product }}</td>
                        <td>{{ $total_order }}</td>
                    </tr>
