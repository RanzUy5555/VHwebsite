...
@section('content')
    <div class="container-fluid mt-5">
        ...
        <br>
        <h3 class="text-center">Monthly Sales</h3> <br>
        <form class="d-print-none" action="{{ route('admin.print.handle') }}" method="GET">
            <div class="input-group">
                <input type="hidden" name="records" value="monthly_sales">
        ...
        <table class="table table-flush table-hover">
            ...
            <tbody>
                @forelse (array_combine($monthly_sales[0], $monthly_sales[1]) as $month => $total_sale)
                    <tr>
                        <td>{{ $month }}</td>
                        <td>{{ $total_sale }}</td>
                    </tr>
